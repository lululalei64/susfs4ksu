...
void susfs_add_sus_path(void __user **user_info) {
    ...
    if (copy_from_user(&info, (struct st_susfs_sus_path __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
    ...
    if (IS_ERR(resolved_pathname)) {
        info.err = PTR_ERR(resolved_pathname);
        goto out_kfree_tmp_buf;
    }
    ...
    if (strstr(resolved_pathname, android_data_path.pathname)) {
        ...
        if (!new_list) { info.err = -ENOMEM; goto out_kfree_tmp_buf; }
...
out_path_put_path:
    path_put(&path);
out_copy_to_user:
    if (copy_to_user(&((struct st_susfs_sus_path __user*)argp)->err, &info.err, sizeof(info.err))) {
        info.err = -EFAULT;
    }
...
void susfs_add_sus_path_loop(void __user **user_info) {
    ...
    {
        info.err = -EINVAL;
        SUSFS_LOGE("path starts with /storage or /sdcard cannot be added by add_sus_path_loop\n");
        goto out_path_put_path;
    }
    ...
    SUSFS_LOGI("target_ino: '%lu', target_pathname: '%s', is added to LH_SUS_PATH_LOOP\n",
                new_list->info.target_ino, new_list->target_pathname);
...
out_copy_to_user:
    if (copy_to_user(&((struct st_susfs_sus_path __user*)argp)->err, &info.err, sizeof(info.err))) {
        SUSFS_LOGE("failed to copy err to user\n");
    }
...
void susfs_add_sus_kstat(void __user **user_info) {
	...
	if (copy_from_user(&info, (struct st_susfs_sus_kstat __user*)argp, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}
	...
	SUSFS_LOGI("is_statically: '%d', target_ino: '%lu', target_pathname: '%s', spoofed_ino: '%lu', spoofed_dev: '%lu', spoofed_nlink: '%u', spoofed_size: '%llu', spoofed_atime_tv_sec: '%ld', spoofed_mtime_tv_sec: '%ld', spoofed_ctime_tv_sec: '%ld', spoofed_atime_tv_nsec: '%ld', spoofed_mtime_tv_nsec: '%ld', spoofed_ctime_tv_nsec: '%ld', spoofed_blksize: '%lu', spoofed_blocks: '%llu', is successfully added to SUS_KSTAT_HLIST\n",
			new_entry->info.is_statically, new_entry->info.target_ino, new_entry->info.target_pathname,
			new_entry->info.spoofed_ino, new_entry->info.spoofed_dev,
			new_entry->info.spoofed_nlink, (unsigned int)new_entry->info.spoofed_size,
			(long)new_entry->info.spoofed_atime_tv_sec, (long)new_entry->info.spoofed_mtime_tv_sec, (long)new_entry->info.spoofed_ctime_tv_sec,
			(long)new_entry->info.spoofed_atime_tv_nsec, (long)new_entry->info.spoofed_mtime_tv_nsec, (long)new_entry->info.spoofed_ctime_tv_nsec,
			(unsigned long)new_entry->info.spoofed_blksize, (unsigned long long)new_entry->info.spoofed_blocks);
#endif

	info.err = 0;

out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_sus_kstat __user*)argp)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
	...
	hash_for_each_safe(SUS_KSTAT_HLIST, bkt, tmp_node, tmp_entry, node) {
		if (!strcmp(tmp_entry->info.target_pathname, info.target_pathname)) {
			...
			if (info.spoofed_size > 0) {
				SUSFS_LOGI("updating spoofed_size from '%llu' to '%llu' for pathname: '%s' in SUS_KSTAT_HLIST\n",
								(unsigned long long)new_entry->info.spoofed_size, (unsigned long long)info.spoofed_size, info.target_pathname);
			...
			if (info.spoofed_blocks > 0) {
				SUSFS_LOGI("updating spoofed_blocks from '%llu' to '%llu' for pathname: '%s' in SUS_KSTAT_HLIST\n",
								(unsigned long long)new_entry->info.spoofed_blocks, (unsigned long long)info.spoofed_blocks, info.target_pathname);
...
void susfs_set_uname(void __user **user_info) {
	...
	if (copy_from_user(&info, (struct st_susfs_uname __user*)argp, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}
	...
	info.err = 0;
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_uname __user*)argp)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
...
void susfs_enable_log(void __user **user_info) {
    ...
    if (!argp) return;
    if (copy_from_user(&info, (struct st_susfs_log __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
        goto out_copy_to_user;
    }
...
out_copy_to_user:
    if (copy_to_user(&((struct st_susfs_log __user*)argp)->err, &info.err, sizeof(info.err))) {
        pr_err("susfs: failed to copy log err to user\n");
    }
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info) {
	...
	info = (struct st_susfs_spoof_cmdline_or_bootconfig *)kzalloc(sizeof(struct st_susfs_spoof_cmdline_or_bootconfig), GFP_KERNEL);
	if (!info) {
		SUSFS_LOGE("kzalloc failed for cmdline info\n");
		return;
	}

	if (copy_from_user(info, (struct st_susfs_spoof_cmdline_or_bootconfig __user*)argp, sizeof(struct st_susfs_spoof_cmdline_or_bootconfig))) {
		ret_err = -EFAULT;
		goto out_copy_to_user;
	}
...
out_copy_to_user:
	...
	if (copy_to_user(&((struct st_susfs_spoof_cmdline_or_bootconfig __user*)argp)->err, &info->err, sizeof(info->err))) {
	}
...
void susfs_add_open_redirect(void __user **user_info) {
	...
	if (copy_from_user(&info, (struct st_susfs_open_redirect __user*)argp, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}
...
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_open_redirect __user*)argp)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
...
void susfs_add_sus_map(void __user **user_info) {
    ...
    struct path path;
    struct inode *inode = NULL;
    void __user *argp = (void __user *)*user_info; // 同样解引用二级指针

    if (!argp) return;

    if (copy_from_user(&info, (struct st_susfs_sus_map __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
        goto out_copy_err;
    }
...
out_copy_err:
    if (copy_to_user(&((struct st_susfs_sus_map __user*)argp)->err, &info.err, sizeof(info.err))) {
        SUSFS_LOGE("failed to copy err to user\n");
    }
...
void susfs_set_avc_log_spoofing(void __user **user_info) {
    ...
    if (copy_from_user(&info, (struct st_susfs_avc_log_spoofing __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
        goto out_copy_to_user;
    }
...
out_copy_to_user:
    if (copy_to_user(&((struct st_susfs_avc_log_spoofing __user*)argp)->err, &info.err, sizeof(info.err))) {
        info.err = -EFAULT;
        SUSFS_LOGE("failed to copy avc spoof err back to user\n");
    }
...
void susfs_get_enabled_features(void __user **user_info) {
	...
	if (copy_from_user(info, (struct st_susfs_enabled_features __user*)argp, sizeof(struct st_susfs_enabled_features))) {
		info->err = -EFAULT;
		goto out_copy_to_user;
	}
...
out_copy_to_user:
	if (copy_to_user((struct st_susfs_enabled_features __user*)argp, info, sizeof(struct st_susfs_enabled_features))) {
		// 这里无法再通过 info->err 报错，因为 copy_to_user 本身就失败了
	}
...
void susfs_show_variant(void __user **user_info) {
    ...
    if (copy_from_user(&info, (struct st_susfs_variant __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
        goto out_copy;
    }
...
out_copy:
    if (copy_to_user((struct st_susfs_variant __user*)argp, &info, sizeof(info))) {
        SUSFS_LOGE("copy_to_user failed\n");
    }
}
...
void susfs_show_version(void __user **user_info) {
    ...
    if (copy_from_user(&info, (struct st_susfs_version __user*)argp, sizeof(info))) {
        info.err = -EFAULT;
        goto out_copy;
    }
...
out_copy:
    if (copy_to_user((struct st_susfs_version __user*)argp, &info, sizeof(info))) {
        SUSFS_LOGE("copy_to_user failed\n");
    }
}
